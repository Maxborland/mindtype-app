...
def _get_hmac_secret() -> str:
    ...
    try:
        ...
        return "MindType2024LicenseSecret"
...
class LicenseManager:
    ...
    def activate(self, license_key: str, online: bool = True) -> tuple[bool, str]:
        """
        Активировать лицензию.

        Args:
            license_key: Лицензионный ключ
            online: Использовать онлайн валидацию (рекомендуется)
        ...
        # Оффлайн активация (только локальная проверка формата)
        # Используется если нет сети
        if not KeyValidator.validate(license_key):
            return False, "invalid_key"

        # Сохраняем лицензию локально (без серверной валидации)
        self._license_data = {
            "license_key": KeyValidator.normalize_key(license_key).replace("-", ""),
            "device_id": self._device_id,
            "validated_at": datetime.now().isoformat(),
            "offline_activation": True,  # Пометка, что нужна онлайн валидация
        }
        self._save_license()

        return True, "activation_success"

    def deactivate(self, online: bool = True) -> bool:
        """
        Деактивировать лицензию.

        Args:
            online: Деактивировать на сервере (рекомендуется)

        Returns:
            True если успешно деактивирована
        """
        if online and self._license_data:
            success, _ = self.deactivate_online()
            if not success:
                # Если не удалось деактивировать онлайн, всё равно удаляем локально
                pass
...
if __name__ == "__main__":
    from .key_validator import generate_license_key

    manager = LicenseManager()

    print("=== License Manager Test ===\n")

    print(f"Device ID: {manager.get_device_id()}")
    print(f"Device Name: {manager.get_device_name()}")
    print()

    # Проверяем текущий статус
    has_access, info = manager.check_access()
    print(f"Has access: {has_access}")
    print(f"Status: {info.status.value}")
    print(f"Is trial: {info.is_trial}")
    print(f"Trial remaining: {info.trial_remaining_days} days")
    print()
    ...
    # Пробуем онлайн активацию
    print("\nTrying online activation...")
    result, message, data = manager.activate_online(test_key)
